...
const Quiz: React.FC = () => {
    ...
    const [quizSource, setQuizSource] = useState<'ai' | 'json' | null>(null);
    ...
    const startNewQuiz = (generatedQuestions: QuizQuestion[]) => {
        if (Array.isArray(generatedQuestions) && generatedQuestions.length > 0) {
            ...
            throw new Error("API không trả về dữ liệu câu hỏi hợp lệ.");
        }
    };
    ...
    const generateQuizQuestions = async () => {
        ...
        try {
            ...
            const quizSchema = {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    options: { type: Type.ARRAY, description: 'Mảng chứa 4 lựa chọn trả lời dạng chuỗi.', items: { type: Type.STRING } },
                    correctAnswer: { type: Type.INTEGER, description: 'Chỉ số (index) của câu trả lời đúng trong mảng options, bắt đầu từ 0.' },
                  },
            ...
            const response = await ai.models.generateContent({
                ...
                contents: "Tạo ra 20 câu hỏi trắc nghiệm về luật chơi, các loại ô, và chiến thuật trong game LeNin Chess. Mỗi câu hỏi phải có 4 lựa chọn trả lời và chỉ có một đáp án đúng. Đảm bảo các câu hỏi bao quát nhiều khía cạnh của trò chơi.",
                config: { responseMimeType: "application/json", responseSchema: quizSchema },
            });
            
            startNewQuiz(JSON.parse(response.text));

        } catch (e) {
            console.error("Error generating quiz questions:", e);
            setError("Rất tiếc, không thể tạo câu hỏi vào lúc này. Vui lòng thử lại sau.");
            setStatus('error');
        }
    };
    ...
    const renderIdle = () => (
        <div className="w-full">
            <p className="text-center text-gray-600 dark:text-gray-400 mb-8">Chọn một cách để bắt đầu bài kiểm tra:</p>
            <div className="grid md:grid-cols-2 gap-8">
                {/* Option 1: AI */}
                <div className="bg-gray-100 dark:bg-gray-800/50 p-6 rounded-lg text-center flex flex-col items-center justify-between ring-1 ring-gray-200 dark:ring-gray-700 hover:ring-brand-gold transition-all">
                    <div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-3">Tạo bộ đề bằng AI</h3>
                        <p className="text-gray-600 dark:text-gray-400 mb-6 min-h-[72px]">Để Gemini tạo ra một bộ 20 câu hỏi ngẫu nhiên và độc đáo về luật chơi LeNin Chess.</p>
                    </div>
                    <button 
                        onClick={generateQuizQuestions}
                        className="bg-brand-gold hover:bg-amber-600 text-gray-900 font-bold py-3 px-8 rounded-full transition-all duration-300 ease-in-out hover:scale-105"
                    >
                        Bắt đầu với AI
                    </button>
                </div>
                ...
                <div className="bg-gray-100 dark:bg-gray-800/50 p-6 rounded-lg text-center flex flex-col items-center justify-between ring-1 ring-gray-200 dark:ring-gray-700 hover:ring-brand-green transition-all">
                    <div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-3">Tải lên bộ đề có sẵn</h3>
                        <p className="text-gray-600 dark:text-gray-400 mb-6 min-h-[72px]">Sử dụng bộ câu hỏi của riêng bạn bằng cách tải lên một tệp JSON theo định dạng cho trước.</p>
                    </div>
...
{`{
  ...
  "history": [
    ...
    const renderFinished = () => {
        ...
        if (percentage >= 90) {
            message = "Xuất sắc! Bạn là một bậc thầy LeNin Chess!";
        ...
        return (
            <div key={currentQuestion} className="w-full animate-question-fade-in">
                <div className="text-center mb-6">
                    {quizSource === 'ai' && (
                        <>
                            <h3 className="text-xl font-bold text-gray-900 dark:text-white">Bộ đề do AI tạo ra</h3>
                            <p className="text-sm text-yellow-600 dark:text-yellow-400/80 mt-1">Lưu ý: Bộ câu hỏi và đáp án có thể sai sót.</p>
                        </>
                    )}
                    {quizSource === 'json' && quizMeta && (
                        <>
                ...
                <div className="mb-6">
                    <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-medium text-gray-900 dark:text-white">Câu hỏi {currentQuestion + 1} / {questions.length}</span>
    ...
    const renderContent = () => {
        switch (status) {
            ...
            case 'idle': default: return renderIdle();
        }
    };
